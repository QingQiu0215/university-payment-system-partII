...
public class DriverII_update 
{
	public static void main(String[] args) 
	{
		System.out.println("Attempting to update some information of bill to the file: Bills.txt.");
		System.out.println("============================================================");	  
		Scanner inputF=null;
		PrintWriter pw=null;
		...
		int billNum=0;
		int index=0;
		/**prompt the users to choose a bill number to update.If the type of bill number is not an long type,enter another one,
		 * if the type is correct,check if it is exist in the bill list, if no, enter another one,if yes,evoke updateBill() to
		...
		while(!done)
		{
		   try
		   {
		     ...
		     index=sub.getArray().indexOf(new Long(billNum));//find the index of the bill number in the ArrayList.
		     if(index==-1)
		     {
		     ...
		     {
		    	 arr.set(index,arr.get(index).updateBill());//alter information.
		         done=true;//let while loop end.
		     }
		   }
		...
		try
		{
		    ...
		    for(int i=0;i<arr.size();i++)
			  pw.println((Bill)a[i]);
		    System.out.println("Updated information was written in the Bills.txt.");
		    pw.close();
		}
		catch(FileNotFoundException e)
		{
